...
const up = async (db: Kysely<any>, dialect: IntrospectorDialect) => {
  ...
  await db.transaction().execute(async (trx) => {
    if (dialect instanceof PostgresIntrospectorDialect) {
      ...
      await sql`CREATE domain pos_int AS Integer CONSTRAINT positive_number CHECK (value >= 0);`
        .execute(
          trx,
        );
    ...
    if (dialect instanceof MysqlIntrospectorDialect) {
      ...
      builder = builder
        ...
        .addColumn(
          "defaulted_nullable_pos_int",
          sql`pos_int`,
          (col) => col.defaultTo(0),
        )
        .addColumn(
          "defaulted_required_pos_int",
          sql`pos_int`,
      ...
      builder = builder
        .addColumn(
          "id",
          "integer",
          (col) => col.autoIncrement().notNull().primaryKey(),
        )
